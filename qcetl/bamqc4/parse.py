...
def extract_flat_fields(input_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for column in [
        ...
        Column.Instrument,
        Column.Library,
        Column.LowQualityReadsMeta,
        Column.MappedReads,
        Column.MismatchBases,
        Column.NonPrimaryReads,
        Column.NonPrimaryReadsMeta,
        ...
        Column.TotalInputReadsMeta,
        Column.TotalReads,
        Column.TotalTargetSize,
        Column.UnmappedReads,
        Column.UnmappedReadsMeta,
...
def parse_int_histogram(input_json: Dict[str, Any]) -> DataFrame:
    ...
    for f in fields:
        df = DataFrame(
            {
                BamQc4IntHistColumn.Value: list(input_json[f].keys()),
                BamQc4IntHistColumn.Count: list(input_json[f].values()),
            }
        )
        df[BamQc4IntHistColumn.Name] = f
        ...
        df[BamQc4IntHistColumn.Value] = df[BamQc4IntHistColumn.Value].astype(
            int
        )
...
def calculate_downsampling_normalization(df: DataFrame) -> Series:
    ...
    Args:
        df: The parsed BamQC4 dataframe
...
def parse_record(
    ...
    insert_size_hist = hist[
        hist[BamQc4IntHistColumn.Name] == "insert size histogram"
    ]
    ...
    if workflow_version >= [4, 0, 3]:
        coverage_hist = hist[
            hist[BamQc4IntHistColumn.Name] == "coverage histogram"
        ]
