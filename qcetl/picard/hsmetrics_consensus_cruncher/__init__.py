...
from qcetl.picard.hsmetrics_consensus_cruncher.parse import parse_record


class HsMetricsConsensusCruncherCache(qcetl.common.Cache):
    """
    The Consensus Cruncher workflow runs the HsMetrics workflow
    """

    def __init__(self):
        super().__init__()
        self.name = "hsmetrics_consensus_cruncher"
        self.columns = {2: {"metrics": HsMetricsConsensusCruncherColumn}}
        self.schema_versions = {
            2: {
                "metrics": {
                    ...
                    HsMetricsConsensusCruncherColumn.MinTargetCoverage: "qf",
                }
            }
        }
        self.input_format = {
        ...
        self.primary_key = {2: {"metrics": [MetricsCol.FileSWID]}}
        self.input_key = {2: ("swid", MetricsCol.FileSWID)}

    def parse_single_record(self, single_input, schema_version):
        metrics, _ = parse_record(single_input["file"])
        return {2: {"metrics": metrics}}[schema_version]
